 ...
 */
public class BullCowRulesUnitTest {
    ...
    public void testAtCreationGetLength() throws Exception {
        //Create new game
        BullCowRules bullCowRules = new BullCowRules();

        //Length should be default
        int length = bullCowRules.getLength();
    ...
    public void testAtCreationGetGameOver() throws Exception {
        ...
        boolean gameOver = bullCowRules.getGameOver();
    ...
    public void testAtCreationGetBulls() throws Exception {
        ...
        //Should throw error
        try {
            int bulls = bullCowRules.getBulls("1234", 1);
    ...
    public void testAtCreationGetCows() throws Exception {
        ...
        //Should throw error
        try {
            int cows = bullCowRules.getCows("1234", 1);
    ...
    public void testSetLengthGood() throws Exception {
        ...
        //Length should be 3
        bullCowRules.setLength(3);
        length = bullCowRules.getLength();
        assertEquals("Length is " + length + " instead of 3.", 3, length, 0);

        //Length should be 5
        bullCowRules.setLength(5);
        length = bullCowRules.getLength();
        assertEquals("Length is " + length + " instead of 5.", 5, length, 0);

        //Length should be 4
        bullCowRules = new BullCowRules();
        length = bullCowRules.getLength();
        assertEquals("Length is " + length + " instead of 4.", 4, length, 0);
    }
    ...
    public void testSetPlayerGood() throws Exception {
        ...
        bullCowRules.setPlayer("1234", 1);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());
        assertEquals("Length is " + bullCowRules.getLength() + " instead of 4.", 4, bullCowRules.getLength(), 0);

        //Player 2, 4 digit number
        bullCowRules.setPlayer("1234", 2);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());
        assertEquals("Length is " + bullCowRules.getLength() + " instead of 4.", 4, bullCowRules.getLength(), 0);

        //Player 1, 3 digit number
        bullCowRules.setLength(3);
        bullCowRules.setPlayer("123", 1);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());
        assertEquals("Length is " + bullCowRules.getLength() + " instead of 3.", 3, bullCowRules.getLength(), 0);

        //Player 1, 5 digit number
        bullCowRules.setLength(5);
        bullCowRules.setPlayer("12345", 1);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());
        assertEquals("Length is " + bullCowRules.getLength() + " instead of 5.", 5, bullCowRules.getLength(), 0);
    }
    ...
    public void testSetPlayerBad() throws Exception {
        ...
        try {
            ...
            assertEquals("Error message doesn't match.", "The number doesn't have " + bullCowRules.getLength() + " digits!", iae.getMessage());
        }

        //Player 1, 4 same digit number
        try {
            bullCowRules.setPlayer("4444", 1);
            assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());
            assertEquals("Length is " + bullCowRules.getLength() + " instead of 5.", 5, bullCowRules.getLength(), 0);
        } catch (IllegalArgumentException iae) {
    ...
    public void testGetResultsGood() throws Exception {
        ...
        BullCowRules bullCowRules = new BullCowRules();
        bullCowRules.setPlayer("1234", 1);
        bullCowRules.setPlayer("9876", 2);
        int bulls;
        int cows;

        //0 Bulls & 0 Cows
        bulls = bullCowRules.getBulls("9876", 1);
        cows = bullCowRules.getCows("9876", 1);
        assertEquals("Bulls is " + bulls + " instead of 0.", 0, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());

        //0 Bulls & 1 Cows
        bulls = bullCowRules.getBulls("1239", 2);
        cows = bullCowRules.getCows("1239", 2);
        assertEquals("Bulls is " + bulls + " instead of 0.", 0, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 1.", 1, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());

        //0 Bulls & 4 Cows
        bulls = bullCowRules.getBulls("4321", 1);
        cows = bullCowRules.getCows("4321", 1);
        assertEquals("Bulls is " + bulls + " instead of 0.", 0, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 4.", 4, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());

        //1 Bulls & 3 Cows
        bulls = bullCowRules.getBulls("9687", 2);
        cows = bullCowRules.getCows("9687", 2);
        assertEquals("Bulls is " + bulls + " instead of 1.", 1, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 3.", 3, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());

        //2 Bulls & 2 Cows
        bulls = bullCowRules.getBulls("1243", 1);
        cows = bullCowRules.getCows("1243", 1);
        assertEquals("Bulls is " + bulls + " instead of 2.", 2, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 2.", 2, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());

        //3 Bulls & 0 Cows
        bulls = bullCowRules.getBulls("9875", 2);
        cows = bullCowRules.getCows("9875", 2);
        assertEquals("Bulls is " + bulls + " instead of 3.", 3, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of false.", false, bullCowRules.getGameOver());

        //4 Bulls & 0 Cows
        bulls = bullCowRules.getBulls("1234", 1);
        cows = bullCowRules.getCows("1234", 1);
        assertEquals("Bulls is " + bulls + " instead of 4.", 4, bulls, 0);
        assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        assertEquals("GameOver is " + bullCowRules.getGameOver() + " instead of true.", true, bullCowRules.getGameOver());
    }
    ...
    public void testGetResultsBad() throws Exception {
        ...
        try {
            cows = bullCowRules.getCows("1234", 2);
            assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        } catch (IllegalArgumentException iae) {
            assertEquals("Error message doesn't match.", "Both player are not ready!", iae.getMessage());
        }
        ...
        try {
            cows = bullCowRules.getCows("1234", 3);
            assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        } catch (IndexOutOfBoundsException iobe) {
        ...
        try {
            cows = bullCowRules.getCows("12345", 2);
            assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        } catch (IllegalArgumentException iae) {
        ...
        try {
            cows = bullCowRules.getCows("4444", 2);
            assertEquals("Cows is " + cows + " instead of 0.", 0, cows, 0);
        } catch (IllegalArgumentException iae) {
            assertEquals("Error message doesn't match.", "The number contains non-unique numbers!", iae.getMessage());
        }
